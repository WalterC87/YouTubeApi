...
var Collections = {
	Videos: Backbone.Collection.extend({
		model: Models.Video,
		url: 'http://gdata.youtube.com/feeds/api/videos?v=2&alt=jsonc&max-results=12',
		parse: function(resp) {
			//console.log('VideosCollection: Recibo del Servidor y hago un parsing de la data');
...
var Views = {
	SingleVideo: Backbone.View.extend({
		...
		render: function() {
			//console.log('SingleVideo: render entero');
	...
	VideosApp: Backbone.View.extend({
		...
		render: function() {
			//console.log('VideosApp: render entero');
